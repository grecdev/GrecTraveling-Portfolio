...
<<<<<<< HEAD
  ...
  const { hotels_db, checkIn_date, checkOut_date, selectedDays } = useContext(
    FormContext
  );
  ...
  const decrementImagePos = () =>
    setCarouselState((carouselState) => ({
      ...carouselState,
      imageCount: carouselState.imageCount - 1,
    }));
  const incrementImagePos = () =>
    setCarouselState((carouselState) => ({
      ...carouselState,
      imageCount: carouselState.imageCount + 1,
    }));
  const setImagePos = (val) =>
    setCarouselState((carouselState) => ({
      ...carouselState,
      imageCount: val,
    }));
  ...
  const displayCarouselBig = (e) => {
      ...
      setTimeout(() => {
        let imagesArray = Array.from(
          document.querySelectorAll(
            ".carousel-big-slider .slider-container .slider-image"
          )
        );
        ...
        const imageWidth = Math.round(
          document
            ...
            .getBoundingClientRect().width + imageMargin
        );
        ...
        imagesArray.forEach((image, index) => {
          ...
          image.style.transform = `translateX(${
            imageWidth * (index - carouselState.imageCount)
          }px)`;

          let imagePos = Math.round(
            parseFloat(image.style.transform.match(/[\d-?]/g, "").join(""))
          );
          ...
          if (carouselState.imageCount === imagesAvailable && index === 0)
            image.style.transform = `translateX(${imageWidth}px)`;
          if (carouselState.imageCount === 0 && index === imagesAvailable)
            image.style.transform = `translateX(${-imageWidth}px)`;
        ...
        document
          .querySelectorAll(".carousel-big-mini .mini-slider-image")
          .forEach((image) => {
            image.style.transition = "none";
    ...
    if (e.currentTarget.tagName === "BUTTON") {
      ...
      setTimeout(() => {
        document
          .querySelectorAll("#room-carousel-container .room-carousel-image")
          .forEach((image) => {
            image.style.transition = "none";
      ...
      document
        .querySelectorAll("#room-carousel-wrapper [data-event-toggle]")
        .forEach((element) => {
          element.setAttribute("data-event-toggle", "false");
          setTimeout(
            () => element.setAttribute("data-event-toggle", "true"),
            carouselState.changeTime
          );
        });
    }
  ...
  const getRoom = () => {
    ...
    const roomItem = hotels_db.filter(
      (hotle) => hotle.roomName === defaultRoomName
    )[0];
  ...
  useEffect(() => {
    // Display the stars
    document
      .querySelectorAll(".room-feedback")
      .forEach(
        (element) =>
      ...
      <main id="hotel-room">
        <section id="room-showcase">
          <div className="container p-2">
            <Link to="/hotels">
              <i className="fas fa-chevron-left"></i> back to hotels
            </Link>
        ...
        <section id="room-preview">
          <div className="container p-2 room-preview-wrapper">
            ...
            <div className="p-1 text-center">
              ...
              <p
                className={
                  getRoom().availability === 1 ? "urgent my-1" : "my-1"
                }
              >
                {getRoom().availability === 1 ? (
                  <i className="fas fa-fire"></i>
                ) : (
                  ""
                )}
                Rooms available: <span>{getRoom().availability}</span>
              </p>
              <p>
                Check-in: <span>{checkIn_date}</span> at{" "}
                <span>{getRoom().checkInHour}</span>
              </p>
              <p>
                Check-out: <span>{checkOut_date}</span> at{" "}
                <span>{getRoom().checkOutHour}</span>
              </p>
              <p>
                Room Price: <span>$ {getRoom().price}</span>
              </p>
              <p>
                Reserved for: <span>{selectedDays}</span>{" "}
        ...
        <section id="room-information" className="mb-1">
          <div className="container room-information-wrapper">
            ...
            <div className="room-information-box">
              ...
              <div className="room-information-content p-1">
                {getRoom().locationServices !== undefined &&
                getRoom().locationServices.foodDrink.length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                        <i className="fas fa-utensils"></i> Food &amp; Drink
                      </h4>

                      <ul>
                        {getRoom().locationServices.foodDrink.map(
                          (item, index) => (
                ...
                {getRoom().locationServices !== undefined &&
                getRoom().locationServices.transportation.length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                      ...
                      <ul>
                        {getRoom().locationServices.transportation.map(
                          (item, index) => (
                            <li key={index}>{item}</li>
                          )
                        )}
                ...
                {getRoom().locationServices !== undefined &&
                getRoom().locationServices.frontDesk_receptionAreaServices
                  .length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                        <i className="fas fa-concierge-bell"></i> Front desk
                        &amp; Area services
                      </h4>

                      <ul>
                        {getRoom().locationServices.frontDesk_receptionAreaServices.map(
                          (item, index) => (
                            <li key={index}>{item}</li>
                          )
                        )}
            ...
            <div className="room-information-box">
              ...
              <div className="room-information-content p-1">
                {getRoom().roomServices !== undefined &&
                getRoom().roomServices.mediaTechnologies.length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                        <i className="fas fa-tv"></i> Media &amp; Technologies
                      </h4>

                      <ul>
                        {getRoom().roomServices.mediaTechnologies.map(
                          (item, index) => (
                            <li key={index}>{item}</li>
                          )
                        )}
                ...
                {getRoom().roomServices !== undefined &&
                getRoom().roomServices.foodsDrinks.length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                        <i className="fas fa-utensils"></i> Food &amp; Drink
                      ...
                      <ul>
                        {getRoom().roomServices.foodsDrinks.map(
                          (item, index) => (
                            <li key={index}>{item}</li>
                          )
                        )}
                ...
                {getRoom().roomServices !== undefined &&
                getRoom().roomServices.roomFacilties.length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                        <i className="fas fa-bed"></i> Room Facilities
                      </h4>

                      <ul>
                        {getRoom().roomServices.roomFacilties.map(
                          (item, index) => (
                            <li key={index}>{item}</li>
                          )
                        )}
                ...
                {getRoom().roomServices !== undefined &&
                getRoom().roomServices.servicesExtras.length > 0 ? (
                  <>
                    <div>
                      <h4 className="heading mb-1">
                        <i className="fas fa-plus"></i> Extra Services
                      </h4>

                      <ul>
                        {getRoom().roomServices.servicesExtras.map(
                          (item, index) => (
                            <li key={index}>{item}</li>
                          )
                        )}
                ...
                getRoom().locationServices["frontDesk_receptionAreaServices"]
                  ...
                  <>
                    <div>
                      ...
                      <ul>
                        {getRoom().locationServices[
                          "frontDesk_receptionAreaServices"
                        ].map((item, index) => (
                          <li key={index}>{item}</li>
                        ))}
                      </ul>
