...
export const FormContextProvider = (props) => {
  const { outerClick, location, changePage, pageLoaded, isMobile } = useContext(
    GlobalContext
  );
  ...
  const setFilteredDatabase = (db, type) => {
    type === "flights" &&
      setDatabase((database) => ({ ...database, appliedFiltered_flights: db }));

    type === "hotels" &&
      setDatabase((database) => ({ ...database, appliedFiltered_hotels: db }));
  };

  const calculateSelectedDays = () => {
    let dateStart = new Date(
      `${
        formState.checkIn_month < 10
          ? "0" + formState.checkIn_month
          : formState.checkIn_month
      }/${
        formState.checkIn_day < 10
          ? "0" + formState.checkIn_day
          : formState.checkIn_day
      }/${formState.checkIn_year}`
    ).getTime();

    let dateEnd = new Date(
      `${
        formState.checkOut_month < 10
          ? "0" + formState.checkOut_month
          : formState.checkOut_month
      }/${
        formState.checkOut_day < 10
          ? "0" + formState.checkOut_day
          : formState.checkOut_day
      }/${formState.checkOut_year}`
    ).getTime();
  ...
  const regexValidation = (e) => {
    ...
    if (e.type === "blur") {
      if (regex.letters.test(target.value.replace(/\n/g, ""))) {
        ...
        target.id === "hotel_destination" &&
          setRegexState((regexState) => ({
            ...regexState,
            hotelDestination_alert: false,
          }));

        e.target.id === "full-name" &&
          setRegexState((regexState) => ({
            ...regexState,
            fullName_alert: false,
          }));
        ...
        e.target.id === "hotel_destination" &&
          setTimeout(() => {
            setRegexState((regexState) => ({
              ...regexState,
              hotelDestination_alert: true,
              hotelsMultiple_alert: false,
            }));
          }, 50);
  ...
  const enableLoading = (loaderType) => {
    if (loaderType === "searchLoader") {
      ...
      setTimeout(
        () =>
          setFormState((formState) => ({ ...formState, searchLoader: false })),
        1000
      );
    }
    ...
    if (loaderType === "filterLoader") {
      ...
      setTimeout(
        () =>
          setFormState((formState) => ({ ...formState, filterLoader: false })),
        1000
      );
    }
  };
  ...
  const displayForm = (e) => {
    if (e.target.tagName === "A") {
      ...
      document
        .querySelectorAll("form input:not(.disabled-input)")
        .forEach((input) => {
          if (input.id !== "flight-passengers" && input.id !== "hotel-people")
            input.classList.remove("input-correct", "wrong-validation");
        });
    }
  };

  const formatCalendarMonth = () =>
    `${date.monthName[currentMonth]} ${currentYear}`;
  ...
  const getMonthDays = (month = currentMonth, year = currentYear) => {
    ...
    totalDays =
      month === 1 ? (leapYear ? 29 : 28) : month31.includes(month) ? 31 : 30;
    ...
    for (let r = 0; r < 6; r++) {
      ...
      for (let c = 0; c < 7; c++) {
        ...
        if (r === 0 && c < firstDayOfMonth()) {
          ...
          cell.innerHTML = `<span>${
            prevMonthDays - (firstDayOfMonth() - c) + 1
          }</span>`;
        ...
        // Current day
        if (
          parseFloat(cell.textContent) === date.currentDay &&
          parseFloat(cell.textContent) <= getMonthDays() &&
        ...
        if (
          ...
          cell.classList.add("before-current-day");

        // Highlight the checkin / checkout day
        if (
          parseFloat(cell.textContent) === formState.checkIn_day &&
          formState.checkIn_month === currentMonth &&
          formState.checkIn_year === currentYear &&
          parseFloat(cell.textContent) <= getMonthDays() &&
          !cell.classList.contains("next-month-day") &&
          !cell.classList.contains("previous-month-day") &&
          !cell.classList.contains("unavailable-day")
        ...
        if (
          parseFloat(cell.textContent) === formState.checkOut_day &&
          formState.checkOut_month === currentMonth &&
          formState.checkOut_year === currentYear &&
          parseFloat(cell.textContent) <= getMonthDays() &&
          !cell.classList.contains("next-month-day") &&
          !cell.classList.contains("previous-month-day") &&
          !cell.classList.contains("unavailable-day")
        ...
        if (
          ...
          cell.classList.add("checkIn-day", "checkOut-day");
        ...
        if (
          ((!cell.classList.contains("next-month-day") &&
            ...
            (cell.classList.contains("previous-month-day") &&
              parseFloat(cell.textContent) >= formState.checkIn_day)) &&
          currentMonth === formState.checkIn_month &&
          currentYear === formState.checkIn_year &&
          (formState.hotelCalendarCheckOut_visible ||
            formState.flightCalendarCheckOut_visible)
        )
          cell.classList.add("before-current-day");
        ...
        if (
          ...
          (formState.hotelCalendarCheckOut_visible ||
            formState.flightCalendarCheckOut_visible) &&
          formState.checkIn_month + 1 === currentMonth
        )
          cell.classList.add("before-current-day");
        ...
        if (
          ...
          parseFloat(cell.textContent) <= formState.checkOut_day &&
          currentMonth === formState.checkIn_month &&
          currentMonth === formState.checkOut_month &&
          currentYear === formState.checkIn_year &&
          currentYear === formState.checkOut_year &&
          !cell.classList.contains("unavailable-day") &&
          !cell.classList.contains("next-month-day") &&
          !cell.classList.contains("previous-month-day")
        )
        ...
        if (
          formState.checkOut_month > formState.checkIn_month &&
          currentMonth === formState.checkIn_month &&
          currentYear === formState.checkIn_year &&
          parseFloat(cell.textContent) >= formState.checkIn_day &&
          !cell.classList.contains("unavailable-day") &&
        ...
        if (
          formState.checkOut_year > formState.checkIn_year &&
          currentMonth > formState.checkIn_month &&
          currentYear === formState.checkIn_year &&
          !cell.classList.contains("unavailable-day") &&
          !cell.classList.contains("next-month-day") &&
  ...
  const selectDate = (e) => {
    ...
    function change(month, year, type) {
      ...
      if (type === "checkin") {
        ...
        setFormState((formState) => ({
          ...
          checkIn_date: `${
            formState.checkIn_day < 10
              ? 0 + formState.checkIn_day.toString()
              : formState.checkIn_day
          } / ${
            formState.checkIn_month < 9
              ? 0 + (formState.checkIn_month + 1).toString()
              : formState.checkIn_month + 1
          } / ${formState.checkIn_year}`,
          hotelCalendarCheckIn_visible: false,
          flightCalendarCheckIn_visible: false,
        }));
      ...
      if (type === "checkout") {
        ...
        setFormState((formState) => ({
          ...
          checkOut_date: `${
            formState.checkOut_day < 10
              ? 0 + formState.checkOut_day.toString()
              : formState.checkOut_day
          } / ${
            formState.checkOut_month < 9
              ? 0 + (formState.checkOut_month + 1).toString()
              : formState.checkOut_month + 1
          } / ${formState.checkOut_year}`,
          hotelCalendarCheckOut_visible: false,
          flightCalendarCheckOut_visible: false,
        }));
    ...
    if (!e.target.classList.contains("table-row") && !multipleSelection) {
      if (
        formState.hotelCalendarCheckIn_visible ||
        formState.flightCalendarCheckIn_visible
      )
        change(checkInMonth, checkInYear, "checkin");

      if (
        formState.hotelCalendarCheckOut_visible ||
        formState.flightCalendarCheckOut_visible
      )
        change(checkOutMonth, checkOutYear, "checkout");
      ...
      if (formState.flightCalendarCheckIn_visible) {
        document
          .getElementById("departing-checkin")
          .classList.remove("wrong-validation");
        document
          .getElementById("departing-checkin")
      ...
      if (formState.flightCalendarCheckOut_visible) {
        document
          .getElementById("returning-checkout")
          .classList.remove("wrong-validation");
        document
          .getElementById("returning-checkout")
          .classList.add("input-correct");
  ...
  const showCalendar = (e) => {
    if (
      e.target.id.toLowerCase().includes("checkin") &&
      formState.checkIn_date.length === 0
    ) {
      setCurrentMonth(date.month);
      setCurrentYear(date.year);
    }
    ...
    if (
      e.target.id.toLowerCase().includes("checkout") &&
      formState.checkOut_date.length === 0
    ) {
      setCurrentMonth(date.month);
      setCurrentYear(date.year);
    }
    ...
    if (e.target.dataset.menuToggle === "on") {
      ...
      if (
        window.matchMedia("(min-width: 481px) and (max-width: 823px)").matches
      ) {
        jump(e.target, {
          duration: 500,
          offset: 0,
      ...
      e.target.id === "departing-checkin" &&
        setFormState((formState) => ({
          ...formState,
          flightCalendarCheckIn_visible: true,
        }));
      e.target.id === "returning-checkout" &&
        setFormState((formState) => ({
          ...formState,
          flightCalendarCheckOut_visible: true,
        }));

      e.target.id === "hotel-checkin" &&
        setFormState((formState) => ({
          ...formState,
          hotelCalendarCheckIn_visible: true,
        }));
      e.target.id === "hotel-checkout" &&
        setFormState((formState) => ({
          ...formState,
          hotelCalendarCheckOut_visible: true,
        }));
  ...
  const showPeopleSelection = (e) => {
    if (e.target.dataset.menuToggle === "on") {
      ...
      e.target.id.includes("flight") &&
        setFormState((formState) => ({
          ...formState,
          peopleSelectionFlights_visible: true,
        }));

      e.target.id.includes("hotel") &&
        setFormState((formState) => ({
          ...formState,
          peopleSelectionHotels_visible: true,
        }));
    ...
    if (e.target.tagName === "A") {
      if (e.target.classList.contains("decrement-people")) {
        ...
        if (formState[decrementButtonValue] > 0) {
          setFormState((formState) => ({
            ...formState,
            peopleTotal: formState.peopleTotal - 1,
            [decrementButtonValue]: formState[decrementButtonValue] - 1,
          }));
        } else
          setFormState((formState) => ({
            ...formState,
            [decrementButtonValue]: 0,
          }));
      }
      ...
      if (e.target.classList.contains("increment-people")) {
        ...
        if (formState[incrementPeopleType] >= 0) {
          setFormState((formState) => ({
            ...formState,
            peopleTotal: formState.peopleTotal + 1,
            [incrementPeopleType]: formState[incrementPeopleType] + 1,
          }));
        }
      }
    }
  ...
  const resetCalendar = () => {
    ...
    if (
      (formState.checkIn_day > formState.checkOut_day &&
        formState.checkIn_month >= formState.checkOut_month &&
        formState.checkIn_year === formState.checkOut_year) ||
      (formState.checkIn_day < formState.checkOut_day &&
        formState.checkIn_month > formState.checkOut_month &&
        formState.checkIn_year >= formState.checkOut_year) ||
      formState.checkIn_year > formState.checkOut_year
      ...
      if (
        document.body.contains(document.querySelector('form[name="flights"]'))
      ) {
        !document
          .querySelector('form[name="flights"]')
      ...
      if (
        document.body.contains(document.querySelector('form[name="hotels"]'))
      ) {
        !document
          .querySelector('form[name="hotels"]')
          .classList.contains("display-none") &&
          document
            .getElementById("hotel-checkout")
            .classList.remove("input-correct");
  ...
  const submitForm = (e) => {
    ...
    if (e.type === "submit") {
      ...
      if (target.getAttribute("name") === "flights") {
        ...
        // By people available
        flightsDb = flightsDb.filter(
          (flight) => flight.people <= formState.peopleTotal
        );
        ...
        formState.flying_to.match(regex.letters) !== null &&
          document.getElementById("flying_to").classList.add("input-correct");
        ...
        if (
          document.querySelectorAll('form[name="flights"] input').length ===
          document.querySelectorAll('form[name="flights"] input.input-correct')
            .length
        ) {
          location !== "/flights" && changePage("/flights");

          if (location === "/flights") {
            if (database.defaultFiltered_flights.length === 0)
          ...
          setTimeout(
            ...
            regexState.alertRemove
          );

          document
            .querySelectorAll('form[name="flights"] input')
            .forEach((input) => {
              !input.classList.contains("input-correct") &&
      ...
      if (target.getAttribute("name") === "hotels") {
        // By destination
        // hotelsDb = hotelsDb.filter((hotel) =>
        //   hotel.destination
        //     .toLowerCase()
        //     .includes(formState.hotel_destination.toLowerCase())
        // );

        hotelsDb = hotelsDb.filter((hotel, index) => {
          // By checkin / checkout month
          // So i was thinking here to automate the months, so i won't need to manually change the months via github
          // A hotel for each month of the year
          // The index of the hotel compared to the checkin / checkout month
          // if the index is less than the current month, obviously we can't select a month
          // So the index will alawys be higher than the current month and less than 12 months
        ...
        // By people available
        hotelsDb = hotelsDb.filter(
          (hotel) => hotel.people <= formState.peopleTotal
        );

        // When we change the page / the form classes doesn't remain assigned on inputs
        formState.hotel_destination.match(regex.letters) !== null &&
          document
            .getElementById("hotel_destination")
            .classList.add("input-correct");
        ...
        // If all fields are correct
        if (
          document.querySelectorAll('form[name="hotels"] input').length ===
          document.querySelectorAll('form[name="hotels"] input.input-correct')
            .length
        ) {
          location !== "/hotels" && changePage("/hotels");

          if (location === "/hotels") {
            if (database.defaultFiltered_hotels.length === 0)
              enableLoading("searchLoader");
          ...
          document
            .querySelectorAll('form[name="hotels"] input')
            .forEach((input) => {
              !input.classList.contains("input-correct") &&
                input.classList.add("wrong-validation");
            });
      ...
      if (target.getAttribute("name") === "contact-us") {
        if (
          document.querySelectorAll('form[name="contact-us"] .input-field')
            .length ===
          document.querySelectorAll('form[name="contact-us"] .input-correct')
            .length
        ) {
          formSubmitted = true;

          document
            .querySelectorAll('form[name="contact-us"] .input-field')
            .forEach((input) => {
              input.value = "";
          ...
          setRegexState((regexState) => ({
            ...regexState,
            contactMultiple_alert: "Message successfully sent",
          }));
          ...
          setRegexState((regexState) => ({
            ...regexState,
            contactMultiple_alert: "All inputs are required",
          }));
      ...
      const consoleStyle =
        "background: #fff; padding: 0.5rem; color: #000; font-weight: bold";
  ...
  useEffect(() => {
    document.body.contains(document.querySelector(".table-body")) &&
      displayMonthDays();
  });
  ...
  useEffect(() => {
    getFetch("https://grecdev.github.io/json-api/flights.json")
      .then((data) =>
        setDatabase((database) => ({ ...database, flights_db: data }))
      )
      .catch((err) => console.log(err));

    getFetch("https://grecdev.github.io/json-api/hotels.json")
      .then((data) =>
        setDatabase((database) => ({ ...database, hotels_db: data }))
      )
      .catch((err) => console.log(err));
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      pageLoaded &&
      location.includes("hotel-room") &&
      database.appliedFiltered_hotels.length === 0
    )
      changePage("/hotels");
  }, [pageLoaded, location]);
